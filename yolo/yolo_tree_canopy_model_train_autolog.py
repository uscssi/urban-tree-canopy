...
def train_with_subprocess(yaml_file, img_sizes, code_base_folder):
    ...
    if 'path' in yaml_data and not os.path.isabs(yaml_data['path']):
        yaml_data['path'] = os.path.abspath(os.path.join(yaml_dir, yaml_data['path'])).replace('\\', '/')
        # Create a new YAML with absolute path
        run_yaml_file = os.path.join(yaml_dir, f"abs_{os.path.basename(yaml_abs_path)}")
        with open(run_yaml_file, 'w', encoding='utf-8') as f:
            yaml.dump(yaml_data, f, sort_keys=False)
        print(f"Created temporary YAML with absolute path: {run_yaml_file}")
    else:
        run_yaml_file = yaml_abs_path
